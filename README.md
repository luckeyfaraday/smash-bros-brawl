# Super Smash Bros. Brawl — browser game
...
screens replace the earlier dark landing page. See [Brawl UI validation](docs/brawl-ui-validation.md).
...
Phone players have touch controls. See [game UI validation](docs/game-ui-validation.md).
...
dummy. See [CPU AI validation and source map](docs/cpu-ai-validation.md).
...
remain reconstructed. See [knockdown validation](docs/knockdown-validation.md)
and the [Build 026 checkpoint](docs/checkpoint-build026.md).
...
follow-up response are reconstructed. See [Link response validation](docs/link-response-validation.md)
...
See [defense validation](docs/defense-validation.md).
...
[down-special validation](docs/down-special-validation.md) and
...
and charge rules. See [side-special-validation.md](docs/side-special-validation.md)
...
his jumping throw arcs. See [grab-validation.md](docs/grab-validation.md) for
...
The assets have already been prepared in this workspace.
Node 24 was used for validation; dependencies are pinned in `package-lock.json`.
...
Open the localhost URL printed by Vite. The current working server is
http://127.0.0.1:5174/ (5173 was already occupied). For a production build:
...
The production build automatically splits fighter animations into JSON files of
at most 20 MiB, then checks every output file against Cloudflare Pages' 25 MiB
limit. The game and `/lab.html` load these files through a small `motion.json`
manifest. Original exports in `public/assets/` remain available to development,
simulation tests and replay tools. Always use `npm run build` to prepare `dist`.

To publish the local build with [Cloudflare Pages Direct Upload](https://developers.cloudflare.com/pages/get-started/direct-upload/):

```powershell
npm run build
npx wrangler login
# Create the project once; choose an available project name.
npx wrangler pages project create brawl-browser --production-branch main
npx wrangler pages deploy dist --project-name brawl-browser --branch main
```

The deploy command uploads the prepared local assets, which are excluded from
Git. To run browser checks against the production files:
...
recordings, not Dolphin captures; see [jab validation](docs/jab-validation.md)
and [neutral aerial validation](docs/aerial-validation.md).
...
| Down special / bombs | Original F.L.U.D.D., bomb, Stone and Thunder assets/scripts; charge storage, transferable bombs, Stone defense and falling Thunder/self-burst work. Common parameters remain reconstructed. See [validation](docs/down-special-validation.md) |
| Side special | Original Cape, Gale Boomerang, Hammer and Skull Bash motion and attack scripts, plus native weapon models. Reflection, return wind, two aerial Hammer swings and chargeable Skull Bash work; common movement, charge, attachment and effects are partly reconstructed. See [validation](docs/side-special-validation.md) |
...
| Tilts and dash attacks | Original scripts, hitboxes, animations and root travel; input selection and transitions reconstructed. Kirby's up-tilt foot intangibility and separate dash hits work. Down-tilt tripping is unfinished. See [validation](docs/directional-attacks-validation.md) |
...
copy. See [recovery validation](docs/recovery-validation.md) for this limitation,
source timing, provisional physics and actual keyboard recovery routes.

## Rebuild assets

The model converter currently requires Windows, 32-bit Windows PowerShell/.NET
Framework, and the BrawlLib DLL from the pinned BrawlCrate v0.42h1 release.
7-Zip is needed only when that release has not yet been unpacked. Python 3.11+
and the project requirements are used for the disc reader.

```powershell
python -m venv .venv
.\.venv\Scripts\python.exe -m pip install -r requirements.txt

# Reuse the extracted fighter/stage PACs; downloads/unpacks BrawlCrate if necessary.
.\.venv\Scripts\python.exe tools\prepare_assets.py

# Or extract the required files from a local image:
.\.venv\Scripts\python.exe tools\prepare_assets.py --rvz 'path\to\game.rvz'
.\.venv\Scripts\python.exe tools\prepare_assets.py --zip 'Super Smash Bros. Brawl (USA) (Rev 1).zip'
```

ZIP input requires enough free disk space for a temporary RVZ plus 512 MB. The
temporary RVZ is removed after extraction. The source ZIP is preserved. The
export manifest records hashes of the source PACs and BrawlLib DLL.

Final Destination uses a separate export from the same local disc:

```powershell
.\.venv\Scripts\python.exe tools/rvz_extract.py 'path\to\game.rvz' --get stage/melee/STGFINAL.PAC --out extract
& "$env:WINDIR\SysWOW64\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -File tools/export_stage.ps1
```

This writes the platform model, textures and stage metadata under
`public/assets/final-destination/`, plus `src/data/final-destination.json` for the
simulation. The export records the source hash and all 17 collision segments.

To regenerate the fighter dumps without re-extracting the disc:

```powershell
python tools\batch_fighters.py
python tools\batch_fighters.py --fighter mario
python tools\build_data.py
```

Schema v2 corrects the misc header and 32-byte hurtbox records, preserves shared
script references, and carries external symbols and raw opcodes. The `frame`
field is only a linear diagnostic preview; it does not execute conditions or loops.
The browser ignores that field and executes raw timers for each fighter’s enabled attacks.
...
Parser tests use the real local Mario PAC. Simulation tests cover all three jab
timelines, combo gates, input consumption during hitlag, hitbox resizing, both
facing directions, interrupted attacks, movement/jumps, bone transforms, capsule
collision and deterministic replay. Aerial tests cover early/late contact,
single-hit behavior across hitbox replacements, fast fall, landing flag boundaries,
recovery locks and cancellation. Battle tests cover both fighters, shields, forward
smash root motion, pushboxes, stocks, respawns, time limits and deterministic CPU
play. Browser tests exercise the complete combo,
short-hop aerial, fast-fall landing, changing timelines, recording/playback,
replaying downloaded JSON outside the browser, asset loading, and desktop/phone
layouts. Screenshots go into `artifacts/`.
...
Its earlier notes remain in [reference validation](docs/reference-validation.md)
and [core build notes](docs/reference-core-build.md).
...
- [Knockdown validation](docs/knockdown-validation.md): source motion, get-up attacks, floor techs, controls, evidence and common-rule limits.
- `src/cpu.ts`: deterministic opponent input decisions.
- `src/roster.ts`: playable fighter identities and asset types.
- [Kirby validation](docs/kirby-validation.md): five air jumps, rapid punches, bone mapping and three-fighter matches.
- [Ranged validation](docs/ranged-validation.md): Mario’s fireball, Link’s bow, independent projectile movement, original weapon animations and reconstruction limits.
- [Thunder Jolt validation](docs/thunder-jolt-validation.md): Pikachu’s original ball/spark assets, target-specific damage and reconstructed surface travel.
- [Inhale validation](docs/inhale-validation.md): Kirby's capture, carrying, escape, original spit event/star, actual keyboard routes and reconstruction limits.
- [Copy validation](docs/copy-validation.md): Kirby's three hats and copied neutral specials, swallow/discard controls, original asset/script mapping and reconstruction limits.
- [Smash validation](docs/smash-validation.md): charge phases, all twelve smashes, source hit groups, captures and reconstruction limits.
- [Pikachu validation](docs/pikachu-validation.md): fourth fighter, headbutt repeat, smash wind-up, electric burst and local matches.
- [Final Cutter validation](docs/final-cutter-validation.md): original rising path, descending blade, landing wave, recovery routes and reconstruction limits.
- [Quick Attack validation](docs/quick-attack-validation.md): directional bursts, original hit branches, stage-clipped attack paths, controls and reconstruction limits.
- [Recovery validation](docs/recovery-validation.md): Mario and Link up specials, source motion and scripts, provisional kinetics and complete browser routes.
- [Stage contact validation](docs/stage-contact-validation.md): solid sides/underside, original fighter body bones, swept contact and a complete recovery route.
- [Link validation](docs/link-validation.md): the second fighter, selection, slash follow-up and mixed matches.
- [Directional attack validation](docs/directional-attacks-validation.md): tilts, dash movement, source timing, Kirby foot intangibility, controls and captures.
- [Directional aerial validation](docs/directional-aerials-validation.md): forward/back/up/down aerials, repeated hits, separate landing hits and controls.
...
- [Battle validation](docs/battle-validation.md): match rules, defense, forward smash and limitations.
- [Defense validation](docs/defense-validation.md): original dodge/ledge motion and scripts, controls and common-rule limits.
- [Ledge validation](docs/ledge-validation.md): original animations, controls and provisional common-state rules.
- [Stage validation](docs/stage-validation.md): current map behavior and rendering limits.
